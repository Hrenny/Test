    ...
    def get_case_count(cls) -> 'TestMetrics':
        ...
        try:
            ...
            if _case_count['total'] > 0:
                # 计算用例成功率
                run_case_data['pass_rate'] = round((_case_count['passed'] + _case_count['skipped']) / _case_count['total'] * 100, 2)
            else:
                # 如果未运行用例，则成功率为0
            ...
            raise FileNotFoundError("程序中检查到您未生成allure报告，\n通常可能导致的原因是allure环境未配置正确，") from exc
