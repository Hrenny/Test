...
class WeChatSend:
    ...
    def send_text(self, content, mentioned_mobile_list=None):
        """
        发送文本类型通知
        :param content: 文本内容最长不超过2048个字节，必须是utf8编码
        :param mentioned_mobile_list: 手机号列表，提醒手机号对应的群成员（@某个成员），@all表示提醒所有人
        :return:
        """
        _data = {'msqtype': 'text', 'text': {'content': content, 'mentioned_list': None, 'mentioned_mobile_list': mentioned_mobile_list}}
        if mentioned_mobile_list is None or isinstance(mentioned_mobile_list, list):
            # 判断手机号码列表中得数据类型，如果为int类型，发送消息会乱码
    ...
    def send_markdown(self, content):
        """发送MarkDown类型消息"""
        _data = {'msqtype': 'markdown', 'markdown': {'content': content}}
        res = requests.post(url=config.wechat.webhook, json=_data, headers=self.headers)
        if res.json()['errcode'] != 0:
            ERROR.logger.error(res.json())
