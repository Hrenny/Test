...
class AssertUtil:
    ...
    def get_assert_data(self):
        assert self.assert_data is not None, ("'%s' should either include a 'assert_data' attribute," % self.__class__.__name__)
    ...
    def get_assert_type(self):
        assert 'AssertType' in self.get_assert_data.keys(), ("断言数据：'%s'中缺少'AsserType'属性" % self.get_assert_data)
    ...
    def sql_switch_handle(self):
        ...
        if self.sql_switch is False:
            WARNING.logger.warning('检测到数据库为关闭状态，程序跳过断言，断言值为：%s' % self.get_assert_data)
