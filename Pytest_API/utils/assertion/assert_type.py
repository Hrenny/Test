...
def length_equals(check_value: Text, expect_value: int, message: Text = ""):
    """判断长度是否相等"""
    assert isinstance(expect_value, int), 'expect_value 需要为int类型'
...
def length_greater_than(check_value: Text, expect_value: Union[int, float], message: Text = ''):
    """判断长度大于"""
    assert isinstance(expect_value, (float, int)), 'expect_value 需要为float/int类型'
    assert len(str(check_value)) > expect_value, message


def length_greater_than_or_equals(check_value: Text, expect_value: Union[int, float], message: Text = ""):
    """判断长度大于等于"""
    assert isinstance(expect_value, (int, float)), 'expect_value 需要为float/int类型'
    assert len(check_value) >= expect_value, message
...
def length_less_than_or_equals(check_value: Text, expect_value: Union[int, float], message: Text = ""):
    """判断长度小于等于"""
    assert isinstance(expect_value, (int, float)), 'expect_value 需要为float/int类型'
    assert len(check_value) <= expect_value, message
...
def contains(check_value: Any, expect_value: Any, message: Text = ''):
    """判断期望结果内容包含在实际结果中"""
    assert isinstance(check_value, (list, tuple, dict, str, bytes)), 'expect_value 需要为list/tuple/dict/str/bytes类型'
    assert expect_value in check_value, message


def contains_by(check_value: Any, expect_value: Any, message: Text = ''):
    """判断实际结果内容包含在期望结果中"""
    assert isinstance(expect_value, (list, tuple, dict, str, bytes)), 'expect_value 需要为list/tuple/dict/str/bytes类型'
